...
static cl::opt<bool> GetDebugInfo("get-debug-info",
                       cl::init(false));
static cl::opt<bool> MapAnnotations("map-annotations", 
                        cl::init(false));
...
static bool inSameLoop(const Value *Base, const Value *Ptr, LoopInfo &LI)
{
  if (isa<PHINode>(Base)) {
...
static void generateMorePointerPairs(Function *F, 
  ...
  for(auto Pair : MustAliasMap) {
    ...
    Instruction *VI2 = dyn_cast<Instruction>(P1);

    if((!inSameLoop(P1, V1, *LI)) || (VI1 && VI2 && !PDT.dominates(VI1->getParent(), VI2->getParent()))) {
      continue;
    }

    Instruction *VI3 = dyn_cast<Instruction>(V2);
    Instruction *VI4 = dyn_cast<Instruction>(P2);

    if((!inSameLoop(P2, V2, *LI)) || (VI3 && VI4 && !PDT.dominates(VI3->getParent(), VI4->getParent()))) {
      continue;
    }
...
static void insertNoAliasScopedMetadata(Function *F, SLVAPass &SLVA, DenseMap<std::pair<int, int>, 
  unsigned long long> &AliasMap, DenseMap<unsigned, Value*> &IDValueMapping, 
  DenseMap<std::pair<int, int>, unsigned long long> &NoAliasDuplicateCheckSet, 
  DenseMap<Value *, uint64_t> PtrSizeMap, DenseSet<Value *> MemTransSrcOperand) {
  MDBuilder MDB(F->getContext());
  MDNode *NewDomain = MDB.createAnonymousAliasScopeDomain("DynAADomain");
  StringRef Name = "DynAliasScope";
  DenseMap<Value*, MDNode*> InstrScopes;

  for (auto NoAliasPair : AliasMap) {
    std::pair<int, int> IDPair = NoAliasPair.first.first < NoAliasPair.first.second ? 
    make_pair(NoAliasPair.first.first, NoAliasPair.first.second) : 
    make_pair(NoAliasPair.first.second, NoAliasPair.first.first);
    if(NoAliasDuplicateCheckSet.count(IDPair)) {
      assert(NoAliasPair.second == NoAliasDuplicateCheckSet[IDPair]);
    }
    ...
    assert(PtrSizeMap[Ptr1]);
    assert(PtrSizeMap[Ptr2]);
    uint64_t Sz1 = PtrSizeMap[Ptr1];
    uint64_t Sz2 = PtrSizeMap[Ptr2];
    if (Size < Sz1 || Size < Sz2) {
      continue;
    }

    if(MemTransSrcOperand.count(Ptr1) || MemTransSrcOperand.count(Ptr2)) {
      continue;
    }

    bool MemAccessInst1 = false;
    for(auto U : Ptr1->users()) {
      if (auto I = dyn_cast<Instruction>(U)) {
        if(SLVA.canTrackAliasInInst(I, true)) {
          MemAccessInst1 = true;
          break;
        }
      }
    ...
    bool MemAccessInst2 = false;
    for(auto U : Ptr2->users()) {
      if (auto I = dyn_cast<Instruction>(U)) {
        if(SLVA.canTrackAliasInInst(I, true)) {
          MemAccessInst2 = true;
          break;
    ...
    if(MemAccessInst1 && MemAccessInst2) {
      if(!InstrScopes.count(Ptr1)) {
        MDNode *NewScope = MDB.createAnonymousAliasScope(NewDomain, Name);
        InstrScopes[Ptr1] = NewScope;
      }
      assert(InstrScopes.count(Ptr1));

      for(auto U : Ptr1->users()) {
        if (auto I = dyn_cast<Instruction>(U)) {
          if(SLVA.canTrackAliasInInst(I, true)) {
            SmallVector<Metadata *, 4> Scopes;
            Scopes.push_back(InstrScopes[Ptr1]);
      ...
      for(auto U : Ptr2->users()) {
        if (auto I = dyn_cast<Instruction>(U)) {
          if(SLVA.canTrackAliasInInst(I, true)) {
            ...
            NoAliases.push_back(InstrScopes[Ptr1]);
	...
	DenseMap<unsigned, Value*> &IDValueMapping, DenseMap<std::pair<int, int>, unsigned long long> &NoAliasDuplicateCheckSet)
{
    ...
    std::pair<int, int> IDPair = AliasPair.first < AliasPair.second ? 
    make_pair(AliasPair.first, AliasPair.second) : make_pair(AliasPair.second, AliasPair.first);
		if(NoAliasDuplicateCheckSet.count(IDPair)) {
...
static void getAnnotationsInFile(SLVAPass &SLVA, int whichPair, int ID1, int ID2, 
  DenseMap<unsigned, Value*> IDValueMapping) {
  Value *Ptr1 = SLVA.getIDValue(ID1, IDValueMapping);
  Value *Ptr2 = SLVA.getIDValue(ID2, IDValueMapping);
  if(whichPair == 0) {
  ...
  if (isa<Argument>(Ptr1)) {
    errs() << "(ID1: " << ID1 << " ARG: " << *Ptr1 << " , ";
  }
  else if (isa<Instruction>(Ptr1)) {
    auto DI = cast<Instruction>(Ptr1)->getDebugLoc();
    if (DI) {
      errs() << "(ID1: " << ID1 << " LINE: " << DI.getLine() << " COL: " << DI.getCol() <<  " VAL:" << *Ptr1 << " , ";
    }
  }
  else {
  ...
  if (isa<Argument>(Ptr2)) {
    errs() << " ID2: " << ID2 << " ARG: " << *Ptr2 << ") \n";
  }
  else if (isa<Instruction>(Ptr2)) {
    auto DI = cast<Instruction>(Ptr2)->getDebugLoc();
    if (DI) {
      errs() << " ID2: " << ID2 << " LINE: " << DI.getLine() << " COL: " << DI.getCol() <<  " VAL:" << *Ptr2  << ") \n";
    }
  }
  else {
    assert(0 && "Invalid pointer.");
...
static void mapAnnotationsToSource(unsigned FID, Function *F, SLVAPass &SLVA, DenseMap<unsigned, Value*> IDValueMapping, 
  DenseMap<int, int> MustAliasMap, DenseMap<std::pair<int, int>, unsigned long long> NoAliasMap, 
  DenseMap<int, set<int>> StaticMustAliasMap, DenseMap<Value *, uint64_t> PtrSizeMap) {
  errs() << "Feedback to the user for the function " << F->getName() << " (FID: " << FID << ")\n";
  errs() << "Mapping annotations to source.\n";
  for(auto MustAliasPair : MustAliasMap) {
    getAnnotationsInFile(SLVA, 0, MustAliasPair.first, MustAliasPair.second, IDValueMapping);
  }
  ...
  for (auto NoAliasPair : NoAliasMap) {
    ...
    uint64_t Sz1 = PtrSizeMap[Ptr1];
    uint64_t Sz2 = PtrSizeMap[Ptr2];
    if (NoAliasPair.second < Sz1 || NoAliasPair.second < Sz2) {
      continue;
    }
    getAnnotationsInFile(SLVA, 1, NoAliasPair.first.first, NoAliasPair.first.second, IDValueMapping);
  }

  errs() << "IDs that are static aliases.\n";
  for(auto StaticMustAliasEle : StaticMustAliasMap) {
    auto ID1 = StaticMustAliasEle.first;
    errs() << "( ID: " << ID1 << " ";
    for(auto StaticMustAliasID : StaticMustAliasEle.second) {
      errs() << ", ID: " << StaticMustAliasID << " ";
    }
    errs() << " )\n";
  }
  errs() << "\n\n";
...
static void getStaticMustAliases(SLVAPass &SLVA, AAResults &AA, 
  DenseMap<unsigned, Value*> IDValueMapping, DenseMap<int, set<int>> &StaticMustAliasMap) {
  for (auto MapElem1 : IDValueMapping) {
    auto ID1 = MapElem1.first;
    auto P1 = MapElem1.second;
    
    for (auto MapElem2 : IDValueMapping) {
      auto ID2 = MapElem2.first;
      auto P2 = MapElem2.second;
      if(ID2 > ID1) {
        const MemoryLocation &M1 = MemoryLocation(P1, LocationSize::unknown());
        const MemoryLocation &M2 = MemoryLocation(P2, LocationSize::unknown());
        AliasResult AR = AA.alias(M1, M2);
        if(AR == MustAlias) {
          StaticMustAliasMap[ID1].insert(ID2);
        }
      }
    }
  }
...
bool GetDynamicAAResults::runOnModule(Module &M) {
  ...
  for (Module::iterator F = M.begin(); F != M.end(); ++F) {
    ...
    DenseMap<Value *, uint64_t> PtrSizeMap;
    DenseSet<Value*> FreeOperand;
    DenseSet<Value*> MemTransSrcOperand;
    ...
    if (Aliases.count(FID)) {
          ...
          if (SLVA.canTrackAliasInInst(Instr, true)) {
            ...
            if(isa<AnyMemTransferInst>(Instr)) {
              ...
              if(auto *AI = dyn_cast<AnyMemTransferInst>(Instr)) {
                ...
                if(!isa<Constant>(Loc.Ptr)) {
                  assert(Loc.Ptr->getType()->isPointerTy());
                  MemTransSrcOperand.insert(const_cast<Value *>(Loc.Ptr));
                  PtrOperands.insert(const_cast<Value *>(Loc.Ptr));
                  if(Loc.Size.hasValue()) {
                    PtrSizeMap[const_cast<Value *>(Loc.Ptr)] = Loc.Size.getValue();
                  }
      ...
      generateMorePointerPairs(Func, &DT, IDValueMapping, ValueIDMapping, AddedPairs, LivePairOperands, LivePairBases, Bases, MustAliasMap, NoAliasMap, MayAliasSet, PtrSizeMap, &AC, &LI);

      if(MapAnnotations) {
        DenseMap<int, set<int>> StaticMustAliasMap;
        getStaticMustAliases(SLVA, AA, IDValueMapping, StaticMustAliasMap);
        mapAnnotationsToSource(FID, Func, SLVA, IDValueMapping, MustAliasMap, NoAliasMap, StaticMustAliasMap, PtrSizeMap);
      }
      ...
      if(GetDebugInfo) {
        ...
        if (!NoAliasMap.empty() || !MayAliasSet.empty()) {
          DenseMap<std::pair<int, int>, unsigned long long> NoAliasDuplicateCheckSet, NoAliasDuplicateCheckSet1;
          DenseSet<std::pair<int, int>> MayAliasDuplicateCheckSet;
          insertNoAliasScopedMetadata(Func, SLVA, NoAliasMap, IDValueMapping, NoAliasDuplicateCheckSet, PtrSizeMap, MemTransSrcOperand);
          if(NoAliasDuplicateCheckSet.size() != NoAliasMap.size()) {
            for (auto NoAliasPair : NoAliasMap) {
              std::pair<int, int> IDPair = NoAliasPair.first.first < NoAliasPair.first.second ? 
              make_pair(NoAliasPair.first.first, NoAliasPair.first.second) : 
              make_pair(NoAliasPair.first.second, NoAliasPair.first.first);
              ...
              Value *Ptr2 = SLVA.getIDValue(NoAliasPair.first.second, IDValueMapping);
              if(!NoAliasDuplicateCheckSet.count(IDPair)) {
                if(TmpPtrOperands.count(Ptr1) || TmpPtrOperands.count(Ptr2)) {
                  continue;
                }
                else if(TmpPtrOperands1.count(Ptr1) || TmpPtrOperands1.count(Ptr2)) {
                  continue;
                }
                else if(FreeOperand.count(Ptr1) || FreeOperand.count(Ptr2)) {
                  continue;
                }
                else if(!PtrOperands.count(Ptr1) || !PtrOperands.count(Ptr2)) {
                  continue;
                }
                else if(MustAliasMap.count(NoAliasPair.first.first) || MustAliasMap.count(NoAliasPair.first.second)) {
                  continue;
                }
                else if(MemTransSrcOperand.count(Ptr1) || MemTransSrcOperand.count(Ptr2)) {
                  continue;
                }
                else {
                  assert(PtrSizeMap[Ptr1]);
                  ...
                  errs() << "Missed by ScopedNoAlias " << *Ptr1 << " " << *Ptr2 << "\n";
                  assert(0 && "Pointer pair missed\n");
                }
              }
            }
          }
          insertNoAliasMetadata(Func, NoAliasMap, IDValueMapping, NoAliasDuplicateCheckSet1);
          insertMayAliasMetadata(Func, MayAliasSet, IDValueMapping, NoAliasDuplicateCheckSet1, MayAliasDuplicateCheckSet);
