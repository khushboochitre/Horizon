...
void AAEvaluator::runInternal(Function &F, AAResults &AA) {
  ...
  int64_t perNoAlias = 0, perMustAlias = 0, perMayAlias = 0, perPartialAlias = 0;
  ++FunctionCount;
  ...
  int64_t AliasSum =
      perNoAlias + perMayAlias + perPartialAlias + perMustAlias;
  errs() << "===== Per-Function Alias Analysis Evaluator Report =====\n";
  errs() << "Function:: " << F.getName() << "\n";
  if (AliasSum == 0) {
    errs() << "  Alias Analysis Evaluator Summary: No pointers!\n";
  } else {
    errs() << "  " << AliasSum << " Total Alias Queries Performed\n";
    errs() << "  " << perNoAlias << " no alias responses ";
    PrintPercent(perNoAlias, AliasSum);
    errs() << "  " << perMayAlias << " may alias responses ";
    PrintPercent(perMayAlias, AliasSum);
    errs() << "  " << perPartialAlias << " partial alias responses ";
    PrintPercent(perPartialAlias, AliasSum);
    errs() << "  " << perMustAlias << " must alias responses ";
    PrintPercent(perMustAlias, AliasSum);
    errs() << "  Alias Analysis Evaluator Pointer Alias Summary: "
           << perNoAlias * 100 / AliasSum << "%/"
           << perMayAlias * 100 / AliasSum << "%/"
           << perPartialAlias * 100 / AliasSum << "%/"
           << perMustAlias * 100 / AliasSum << "%\n";
  }
  errs() << "\n";
}
