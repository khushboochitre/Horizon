...
static cl::opt<bool> DisableGetDynAA("disable-get-dynaa",
                       cl::init(false));
static cl::opt<string> StatsFileName("stats-file-name");
...
AliasResult AAResults::alias(const MemoryLocation &LocA,
  ...
  if(!StatsFileName.empty()) {
    ...
    if(F && F->hasName()) {
      ...
      assert(LogDirEnv);
      string FileName = LogDirEnv;
      FileName += "/logs/" + StatsFileName + "_stats.txt";

      unsigned long long Tot, No, May, Par, Must;
      Tot = 1;
      No = Result == NoAlias ? 1 : 0;
      May = Result == MayAlias ? 1 : 0;
      Par = Result == PartialAlias ? 1 : 0;
      Must = Result == MustAlias ? 1 : 0;
      string writestr;
      bool found = false;
      ...
      if(access(FileName.c_str(), F_OK ) != -1) {
        ...
        if(!File1.good()) {
          errs() << "No file found. Filename:: " << FileName << "\n";
          exit(0);
        }

        if (!File1.is_open()) {
          errs() << "Error in opening " << FileName << "\n";
          exit(0);
        }
        ...
        while (getline(File1, str)) {
          ...
          string Fname;
          ss >> Fname;
          //if(str.find(F->getName().str(), 0) != string::npos) {
          if(F->getName().equals(Fname)) {
            found = true;
            string TotPair, NoPair, MayPair, ParPair, MustPair;
            ss >> TotPair;
            ss >> NoPair;
            ss >> MayPair;
            ss >> ParPair;
            ...
            May += stoull(MayPair);
            Par += stoull(ParPair);
            Must += stoull(MustPair);
            assert(Tot == (No + May + Par + Must));
            writestr += F->getName().str() + " " + to_string(Tot) + " " 
            + to_string(No) + " " + to_string(May) + " " + to_string(Par) + " " + to_string(Must) + "\n";
          }
          else {
            writestr += str + "\n";
          }
