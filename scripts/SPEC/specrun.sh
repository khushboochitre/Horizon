...
do
        ...
        elif [ $arg == "map-annotation" ];
        then
                echo "Map Annotations"
                runcpu --config=map-annotation --noreportable --tune=base --copies=1 --iteration=1 --action=clobber --size=$SIZE $BENCHMARK
                runcpu --config=map-annotation --noreportable --tune=base --copies=1 --iteration=1 --size=$SIZE $BENCHMARK
        else
